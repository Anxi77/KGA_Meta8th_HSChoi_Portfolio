...
public class SPUM_SpriteEditManager : MonoBehaviour
{
    ...
    public void SetPivot(SpriteRenderer _sprite)
    {
        ...
        var dataProvider = UnityEditor.AssetImporter.GetAtPath(path) as UnityEditor.U2D.Sprites.ISpriteEditorDataProvider;
        if (dataProvider != null)
        {
            dataProvider.InitSpriteEditorDataProvider();
            var textureDataProvider = dataProvider.GetDataProvider<ITextureDataProvider>();
            int textureActualWidth, textureActualHeight;
            textureDataProvider.GetTextureActualWidthAndHeight(out textureActualWidth, out textureActualHeight);

            var spriteDataProvider = dataProvider.GetDataProvider<ISpriteEditorDataProvider>();
            ...
            if (spriteRects.Length > 1)
            {
                for (var i = 0; i < spriteRects.Length; i++)
                {
                    ...
                    {
                        float tXSize = spriteRect.rect.size.x;
                        float tYSize = spriteRect.rect.size.y;
                        ...
                        spriteRect.pivot = new Vector2(rX, rY);
