...
public class SkillDataEditor : EditorWindow
{
    ...
    private SkillDataContainer dataContainer;
    ...
    private void DrawPrefabSettings()
    {
        ...
        {
            Debug.Log($"Icon changed for skill: {currentSkill.metadata.Name}"); // 디버그 로그 추가
    ...
    private void AddNewLevelStat()
    {
        ...
        // 이전 레벨의 스탯을 복사하여 새 레벨 스탯 생성
        var newStat = levelStats.Count > 0
            ? levelStats[levelStats.Count - 1].Clone()
            : new SkillStatData();

        // 새 레벨의 스탯 증가
        CopyStatsFromPrevious(levelStats[levelStats.Count - 1], newStat);
        ...
        Debug.Log($"Added new level {newLevel} stats for skill {currentSkill.metadata.Name}");
    }
    ...
    private void SaveIconToResources(Sprite icon, SkillID skillId)
    {
        ...
        try
        {
            Debug.Log($"Starting to save icon for SkillID: {skillId}");
            ...
            Debug.Log($"Source texture size: {sourceTexture.width}x{sourceTexture.height}");
            ...
            if (!sourceTexture.isReadable)
            {
                ...
                Debug.Log($"Source texture path: {sourcePath}");
                ...
                if (importer != null)
                {
                    ...
                    Debug.Log("Texture made readable");
                }
                else
                {
            ...
            Debug.Log($"Sprite rect: x={spriteRect.x}, y={spriteRect.y}, w={spriteRect.width}, h={spriteRect.height}");
            ...
            try
            {
                ...
                newTexture.Apply();
                Debug.Log("Created new texture with copied pixels");

                // 파일 저장
                byte[] bytes = newTexture.EncodeToPNG();
                File.WriteAllBytes(destinationPath, bytes);
                Debug.Log($"Saved PNG file to: {destinationPath}");
                ...
                {
                    Debug.Log("Configuring texture importer settings");
                    ...
                    destinationImporter.filterMode = FilterMode.Bilinear;
                    destinationImporter.textureCompression = TextureImporterCompression.Uncompressed;
                    destinationImporter.SaveAndReimport();
                }

                // 새 스프라이트 로드
                Sprite newSprite = AssetDatabase.LoadAssetAtPath<Sprite>(assetPath);
                if (newSprite != null)
                {
                    currentSkill.icon = newSprite;
                    currentSkill.metadata.Icon = newSprite;
                    Debug.Log($"Successfully loaded new sprite from: {assetPath}");
                }
                else
                {
            ...
            {
                Debug.LogError($"Error during texture processing: {e.Message}\n{e.StackTrace}");
            }
        }
        catch (System.Exception e)
        {
    ...
    {
        // Resources 폴더에서 컨테이너 로드
        dataContainer = Resources.Load<SkillDataContainer>("SkillDataContainer");

        if (dataContainer == null)
        {
            // 컨테이너가 없으면 새로 생성
            dataContainer = CreateInstance<SkillDataContainer>();

            // Resources 폴더가 없다면 생성
            if (!Directory.Exists("Assets/Resources"))
            {
                Directory.CreateDirectory("Assets/Resources");
            }

            string containerPath = "Assets/Resources/SkillDataContainer.asset";
            AssetDatabase.CreateAsset(dataContainer, containerPath);
            AssetDatabase.SaveAssets();
            Debug.Log("Created new SkillDataContainer");
        }

        // 데이터 로드
        skillList = new List<SkillData>(dataContainer.skillList ?? new List<SkillData>());

        // 스킬 스탯 데이터 로드
        skillStatsList = dataContainer.GetSkillStatsList();
        ...
        Debug.Log($"Loaded {skillList.Count} skills from container");
    }
    ...
    private void SaveToDataContainer()
    {
        if (dataContainer != null)
        {
            try
            {
                ...
                {
                    var stats = skillStats.Value;
                    if (stats != null)
                    {
                        // 레벨 순서대로 정렬
                        stats.Sort((a, b) => a.level.CompareTo(b.level));

                        // 각 레벨의 스탯이 제대로 설정되어 있는지 확인
                        for (int i = 0; i < stats.Count; i++)
                        {
                            if (stats[i].level != i + 1)
                            {
                                Debug.LogWarning($"Fixed level mismatch for skill {skillStats.Key}: " +
                                               $"Level {stats[i].level} -> {i + 1}");
                                stats[i].level = i + 1;
                            }
                        }
                    }
                ...
                // 데이터 저장
                dataContainer.SaveData(skillList, skillStatsList);

                // 리소스 레퍼런스 저장
                foreach (var skill in skillList)
                {
                    if (skill.metadata == null || skill.metadata.ID == SkillID.None) continue;
                    SaveSkillResources(skill, new ResourceReferenceData());
                }

                EditorUtility.SetDirty(dataContainer);
                AssetDatabase.SaveAssets();
                Debug.Log("Successfully saved data to container");
            }
            catch (System.Exception e)
            {
                Debug.LogError($"Error saving to container: {e.Message}\n{e.StackTrace}");
            }
        }
    }
}